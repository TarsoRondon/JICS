...
export async function listarAdmins(req, res) {
  try {
    ...
    const message = err?.code === 'ER_BAD_FIELD_ERROR'
      ? 'Tabela admins desatualizada. Aplique a migracao de schema.'
      : 'Erro ao carregar administradores.';
    return jsonErro(res, 500, message);
  }
}
...
export async function ativarAdmin(req, res) {
  try {
    ...
    console.error('admins.status', err);
    const message = err?.code === 'ER_BAD_FIELD_ERROR'
      ? 'Tabela admins desatualizada. Aplique a migracao de schema.'
      : 'Erro ao atualizar status do administrador.';
    return jsonErro(res, 500, message);
  }
}
